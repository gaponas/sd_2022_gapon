...
class Parser:
    ...
    def substitute_vars(self, line):
        """
        Выполняет подстановку значений локальных переменных в строку-запрос
        :param line: str, строка-запрос
        ...
        for var in find_vars:
            ...
            if var_value is None:
                raise NoVariableInMemory(f"{var_name} not in memory")
            line = line.replace('$' + var_name, var_value)
        return line

    def split(self, line):
        """
        Делит строку на токены -- части команд.
        Если передана строка присваивания значения переменной -- выполняет присваивание
        :param line: str, строка
        :return: list[str], список строк; если выполнили присваивание -- список пуст
        """
        res_of_split = []
    ...
    def parse(self, line):
        """
        Функция парсинга строки запроса в команды интерпритатора
        :param line: str, строка запроса
        :return: list[CommandInterface], массив команд
        """
